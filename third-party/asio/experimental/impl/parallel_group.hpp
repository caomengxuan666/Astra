        ...
        namespace detail {
            ...
            struct ranged_parallel_group_completion_handler {
                ...
                void invoke(asio::detail::index_sequence<I...>) {
                    typedef typename parallel_op_signature_as_tuple<
                            typename ranged_parallel_group_signature<
                                    ...
                                    Allocator>::raw_type>::type vectors_type;
                ...
                std::vector<std::size_t,
                            ASIO_REBIND_ALLOC(Allocator, std::size_t)>
                        completion_order_;
                std::deque<op_result_type,
                           ASIO_REBIND_ALLOC(Allocator, op_result_type)>
                        args_;
            };
            ...
            struct ranged_parallel_group_op_handler {
                ...
                ranged_parallel_group_op_handler(
                        std::shared_ptr<ranged_parallel_group_state<
                                Condition, Handler, Op, Allocator>>
                                state,
                        std::size_t idx)
                    : state_(std::move(state)),
                      idx_(idx) {
            ...
            struct ranged_parallel_group_op_handler_with_executor : ranged_parallel_group_op_handler<Condition, Handler, Op, Allocator> {
                ...
                ranged_parallel_group_op_handler_with_executor(
                    ...
                    : ranged_parallel_group_op_handler<Condition, Handler, Op, Allocator>(
                              std::move(state), idx) {
                    cancel_proxy_ =
                            &this->state_->cancellation_signals_[idx].slot().template emplace<cancel_proxy>(this->state_, std::move(ex));
                }
