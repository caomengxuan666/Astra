...
// 通用命令执行
ZEN_API RespValue_C *astra_client_send_command(AstraClient_C *client, const char **argv, int argc);
...
ZEN_API const RespValue_C *astra_resp_value_array_element(const RespValue_C *value, size_t index);
ZEN_API void astra_resp_value_destroy(RespValue_C *value);

// 常用命令封装
ZEN_API RespValue_C *astra_client_ping(AstraClient_C *client);
ZEN_API RespValue_C *astra_client_set(AstraClient_C *client, const char *key, const char *value);
ZEN_API RespValue_C *astra_client_setex(AstraClient_C *client, const char *key, const char *value, int ttl);
ZEN_API RespValue_C *astra_client_get(AstraClient_C *client, const char *key);
ZEN_API RespValue_C *astra_client_mget(AstraClient_C *client, const char **keys, int key_count);
ZEN_API RespValue_C *astra_client_mset(AstraClient_C *client, const MSetItem *items, int item_count);
ZEN_API RespValue_C *astra_client_del(AstraClient_C *client, const char **keys, int key_count);
